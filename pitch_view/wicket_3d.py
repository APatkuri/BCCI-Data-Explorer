...
def plot_wicket_3d(ax,
    ...
    if isinstance(pitch_colour, str):
        ax.zaxis.set_pane_color(colors.to_rgba(pitch_colour, 1))
    else:
        ax.zaxis.set_pane_color(pitch_colour)
    ...
    #     #        zdir="z", size=0.25, usetex=False, angle=np.pi,
    #     #        facecolor='black', edgecolor=outline_colour)
    #     ax.plot([-2, 2], [0, 0], [dist, dist], c=marking_colour, alpha=0.2, ls='--', lw=1)
    #     ax.text(2.1, 0, dist, '{0}m'.format(dist), color=outline_colour, fontproperties=fp, size=14) 
