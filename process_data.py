...
def parse_trajectory_data(dat: str):
    '''Parses trajectory data formatted as <Key>Value pairs into a dictionary.'''
    ...
    pattern = r"<([^>]+)>\s*([^<]*)"
    matches = re.findall(pattern, dat)
    
    # Create dictionary from matches
    myDict = {key.strip(): value.strip() for key, value in matches}
    
    # Remove specific key if it exists
    myDict.pop("Software Version", None)
    return myDict
...
def fill_trajectory_data(ball_data, delivery_data, trajectory_dict):
    ...
    shot_information = delivery_data['shotInformation']
    ...
    ball_data_traj = {'delivery_type': delivery_data['deliveryType'], 'match_day':delivery_data['deliveryNumber']['day'], 
                      'release_speed': trajectory_data['releaseSpeed'], 'initial_angle': trajectory_data['initialAngle'], 
                      'release_x': release_data['x'], 'release_y': release_data['y'], "release_z" : release_data['z'],
                      'bounce_angle': trajectory_data['bounceAngle'], 
                      'cof': trajectory_data['cof'], 'cor': trajectory_data['cor'], 'pbr': trajectory_data['pbr'], 
                      'shot_attacked': shot_information['shotAttacked'], 'shot_played': shot_information['shotPlayed'], 'shot_info': shot_information['shotTypeAdditional'], 
                      'crease_reaction_time': trajectory_data['reactionTime(to crease)'], 'interception_reaction_time': trajectory_data['reactionTime(to interception)'],
...
def processData(ball_data, response):
    ...
    if batting_team_info['batsman']['isRightHanded']:
        ball_data['striker_hand'] = 'R'
    else:
        ball_data['striker_hand'] = 'L'

    if batting_team_info['batsmanPartner']['isRightHanded']:
        ball_data['non_striker_hand'] = 'R'
    else:
        ball_data['non_striker_hand'] = 'L'

    if bowling_team_info['isRightHanded']:
        ball_data['bowler_hand'] = 'R'
    else:
        ball_data['bowler_hand'] = 'L'
