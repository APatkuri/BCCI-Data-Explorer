...
def get_bcci_shot_data(match_id, max_overs):
    ...
    with open(f"./ipl_shot_data/json/{match_id}.json", 'w') as f:
        json.dump(match_data, f, indent=4)

def csv_to_json(match_id):
    df = pd.read_json(f"./ipl_shot_data/json/{match_id}.json")
    ...
    if df.empty:
        ...
        if 'OverHistory' in df.columns:
            ...
            if len(final_df) > 0:
                final_df.to_csv(f"./ipl_shot_data/csv/{match_id}.csv")
...
def ipl_shot_data_json():
    ...
    if os.path.exists(f"./ipl_shot_data/ipl_match_list.csv"):
        existing_df = pd.read_csv(f"./ipl_shot_data/ipl_match_list.csv")
    ...
    with open(f"./ipl_shot_data/ipl_match_list.json", 'w') as f:
...
def match_data_procees(ipl_match_list, new_match_list):
    ...
    live_data_file_name = f"./ipl_shot_data/live_data_file_name.txt"
    ...
    temp_df.to_csv(f'./ipl_shot_data/ipl_match_list.csv', index=False)

    ####################

    # Path to the folder containing the CSV files
    folder_path = f'./ipl_shot_data/csv'  # Path to the 'csv' directory
    ...
    if existing_csv_files:
        ...
        final_df.to_csv(f'./ipl_shot_data/combined_shot_data.csv', index=False)
...
def hawkeye_data():
    ...
    hawkeye_file_name = f"./ipl_shot_data/hawkeyeid_matchid.csv"
    try:
    ...
    temp_df = pd.read_json(f"./ipl_shot_data/ipl_match_list.json", convert_dates=False)
